    ...
    .controller('MyCoursesController', function MyCourseController($scope, $http) { // Läs 2 nedan
        $http.get("courses.json") // Läs 3 nedan
            .then(function successCallback(response) { // Läs 4 nedan
                ...
                $scope.myCourses_Information = data.myCourses_Information;
                $scope.getCourseName = function(asd) {
                    let tmp;
                        for(let s=0; s < data.courses.length; s++){
                        if (asd == data.courses[s].courseCode){
                            tmp = data.courses[s].name;
                        }
                    }
